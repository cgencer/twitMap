var theApp = (function(theApp, undefined) {
	...
	__dirName = __dirName.slice(0, __dirName.lastIndexOf('/'));		// trim another one up, 'cause of source/js/
	app.configure(function () {
		...
		app.use( stylus.middleware( {
			...
			compile : function(str, path) {
				...
				return stylus(str)				// http://learnboost.github.io/stylus/docs/js.html
					.use( nib() )				// http://visionmedia.github.io/nib/
					.use( fontFace() )			// https://github.com/code42day/stylus-font-face
					.use( axis() )				// http://roots.cx/axis/
					.use( fluidity() )			// http://www.fluiditycss.com
					.set('filename', path)
					.set('warn', true)
					.set('compress', true)
			}
